...
async function handleNewMember(guildMember) {
  ...
  const member = await (0, _member.registerMember)(guildMember);
  const name = member.nickname || member.username;

  try {
    await guildMember.send(`Hey ${name}! Welcome to this Discord server 😄`);
    await guildMember.send(`This server is meant exclusively for folks who have purchased the “CSS for JavaScript Developers” course.`);
    await guildMember.send(`Can you please send me the **email address** you used during purchase?`);
  } catch (err) {
